function Scrubber({scrubberEle, parentEle, effectorEle, config: { handleClickOnParent = false, onValueChange = () => {} } = {} } = {}) {
  const CONSTANTS = {
    ERRORS: {
      SCRUBBER_PARENT_AND_EFFECTOR_REQUIRED: new Error('Scrubber, parent and effector elements are required'),
    },
    ...
    function handleMouseMoveOnScrubberParent(event) {
      try {
        ...
        requestAnimationFrame(() => {
          ...
          if (typeof onValueChange === 'function') onValueChange(seekPercentage);
        })
      } catch (error) {
        console.error(error);
      }
